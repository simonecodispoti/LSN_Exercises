...
{ 
  ...
  for(int iblk=1; iblk <= nblk; ++iblk)         // Simulation
  {
    ...
    for(int istep=1; istep <= nstep; ++istep)
    {
      Move();
      Measure();
      //Istant();                               // Print istantaneous values (comment for non-equilibration simulation!)
      Accumulate();                             // Update block averages
      if(istep%10 == 0){
        //ConfXYZ(nconf);                       // Write actual configuration in XYZ format (comment to avoid "filesystem full"!) 
        nconf += 1;
      }
    }
...
//Read input informations
  ...
  //Tail corrections for potential energy and pressure (per unit particle)
...
void Istant()
{
  ...
  Energy.open("Energy.ist", ios::app);
  Pressure.open("Pressure.ist", ios::app);

  double E = walker[iv]/(double)npart + vtail;
  double P = rho * temp + (walker[iw] + (double)npart * ptail) / vol;
...
//g(r): final estimation with averages and errors
    ...
    if(iblk==nblk){    
      ...
      for(int k=igofr; k<igofr+nbins; k++){
        ...
        err_gdir = Error(glob_av[k],glob_av2[k],nblk);
        Gave << setw(wd) << r << setw(wd) << glob_av[k]/(double)nblk << setw(wd) << err_gdir << endl;
      }
      Gave.close();
    }
